...
| Schema drift between models and database | Foreign-key delete rules are now declared in the models, so autogenerate no longer proposes dropping the database's `SET NULL` and `CASCADE` behaviour. Three unrelated drifts remain: the `public_sites.api_key` unique constraint exists only in the database, the `ix_quote_events_user_id` index only in the models, and `users.notification_preferences` is `NOT NULL` in the models but nullable in the database | Autogenerate emits nothing on an unchanged model set, so a generated migration can be trusted without hand-editing | High | Resolve the three remaining drifts one at a time, deciding per case whether the model or the database is right, and review every autogenerated migration until that output is clean |
