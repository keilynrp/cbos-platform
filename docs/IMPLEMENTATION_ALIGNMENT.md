...
- Error Code Registry V1 exists and CI enforces parity between raised codes, registry entries, and the frontend translation map. `projects`, `sales`, and `crm` raise registered codes and their pages render Spanish from them; the other eight modules still return free-text `detail` and reach the user in English
