...
| Schema drift between models and database | Foreign-key delete rules are declared in the models, and `users.notification_preferences` is now `NOT NULL` in the database as the model always claimed. Two drifts remain: the `public_sites.api_key` unique constraint exists only in the database, and the `ix_quote_events_user_id` index only in the models | Autogenerate emits nothing on an unchanged model set, so a generated migration can be trusted without hand-editing | High | Resolve the two remaining drifts one at a time, deciding per case whether the model or the database is right, and review every autogenerated migration until that output is clean |
