...
| Frontend error surfacing | `HR.tsx` and `Projects.tsx` read `e.response.data.detail` in their mutation `onError`, an axios shape this codebase does not use: `lib/api.ts` throws a plain `Error` carrying the backend detail in `message`, so the expression always evaluated to `undefined` and the user only ever saw the generic fallback. Both now read `e.message`. The bug survived because the handlers typed the error as `any`, which disabled the check that would have caught it | Every mutation surfaces the backend's own message when it has one | Medium | Audit the remaining `onError` handlers for the same axios-shaped access, and prefer a typed error over `any` so the compiler can see the mistake |
